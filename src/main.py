...
def raise_200():
    ...
    logger.info('raise money 200 end, total count : %s, success count : %s, failure count : %s', *(total_num, success_num, failure_num))
    error_record.to_csv('../raise_amount_200_error.csv', mode='a+', encoding='utf-8', header=True, index=False, index_label=None)


def raise_300():
    logger.info('raise money 300 start ...')
    total_num, success_num, failure_num = 0, 0, 0
    error_record = pd.DataFrame(columns=[u'手机号', u'额度'])
    file_list = ['../resource/300.xls', ]
    for f in file_list:
        logger.info(f)
        quota = int(f[-7:-4])
    ...
    logger.info('raise money 300 end, total count : %s, success count : %s, failure count : %s', *(total_num, success_num, failure_num))
    error_record.to_csv('../raise_amount_300_error.csv', mode='a+', encoding='utf-8', header=True, index=False, index_label=None)


def raise_400():
    logger.info('raise money 400 start ...')
    total_num, success_num, failure_num = 0, 0, 0
    error_record = pd.DataFrame(columns=[u'手机号', u'额度'])
    file_list = ['../resource/400.xls', ]
    for f in file_list:
        logger.info(f)
        quota = int(f[-7:-4])
        logger.info('start raise 400 money %s ...', quota)
    ...
    logger.info('raise money 400 end, total count : %s, success count : %s, failure count : %s', *(total_num, success_num, failure_num))
    error_record.to_csv('../raise_amount_400_error.csv', mode='a+', encoding='utf-8', header=True, index=False, index_label=None)


def raise_500():
    logger.info('raise money 500 start ...')
    total_num, success_num, failure_num = 0, 0, 0
    error_record = pd.DataFrame(columns=[u'手机号', u'额度'])
    file_list = ['../resource/500.xls', ]
    for f in file_list:
        logger.info(f)
        quota = int(f[-7:-4])
        logger.info('start raise 500 money %s ...', quota)
    ...
    logger.info('raise money 500 end, total count : %s, success count : %s, failure count : %s', *(total_num, success_num, failure_num))
    error_record.to_csv('../raise_amount_500_error.csv', mode='a+', encoding='utf-8', header=True, index=False, index_label=None)
...
threads.append(t2)
t3 = threading.Thread(target=raise_400)
threads.append(t3)
t4 = threading.Thread(target=raise_500)
threads.append(t4)
